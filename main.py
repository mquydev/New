...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]Hᴀsɪʙ Cᴏᴅᴇx [00FF00]___ Hᴀsɪʙ Cᴏᴅᴇx____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @hasib_hi\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
[FFFFFF]██║░░██║██╔══██╗██╔════╝[00FF00]██║[FF8800]██╔══██╗
[FF0000]███████║███████║╚█████╗░[0000FF]██║[FFFF00]██████╦╝
[FFFFFF]██╔══██║██╔══██║░╚═══██╗[00FF00]██║[FF8800]██╔══██╗
[FF0000]██║░░██║██║░░██║██████╔╝[0000FF]██║[FFFF00]██████╦╝
[FFFFFF]╚═╝░░╚═╝╚═╝░░╚═╝╚═════╝░[00FF00]╚═╝[FF8800]╚═════╝

[FF0000]H[0000FF]A[FFFF00]S[FFFFFF]I[00FF00]B"""
...
[b][i][FFA500]telegram: @hasib_hi[/b]

[b][c][FFA500]telegram contact: @hasib_hi[A52A2A]
 
Enjoy the bot my friend.......

[C][B][0000FF] Created by Black666FF
Modified by - @hasib_hi
"""
                        ...
                        """
                                ...
                                try:
                                    ...
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                                                                          ...
                                                                                          # FIXED JOIN COMMAND
                        if inPuTMsG.startswith('h'):
                            # Process h command in any chat type
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: h (team_code)\nExample: h ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
...
# GLOBAL BLOCKED NAMES
                        BLOCKED_NAMES = ["HAsib", "hasib", "hasib 6T9", "Hasib", "Hasid", "Hasib 6t9"]  # Protected names
...
# ==================== /luck COMMAND ====================
                        ...
                        if inPuTMsG.strip().startswith('@evos'):
                            ...
                            # SUCCESS MESSAGE
                            if len(uids) == 1:
                                success_msg = f"[B][C][FFA500]✅ SUCCESS! Evolution emote cycle started!\n🎯 Target: Yourself\n🎭 [00FA9A]Emotes: All 18 evolution emotes\n⏰ Delay: 5 seconds between emotes\n🔄 Cycle: Continuous loop until @sevos\n"
                            else:
                                success_msg = f"[B][C][00FA9A]✅ SUCCESS! Evolution emote cycle started!\n🎯 Targets: Yourself + {len(uids)-1} other players\n[00FFFF]🎭 Emotes: All 18 evolution emotes\n⏰ Delay: 5 seconds between emotes\n🔄 Cycle: Continuous loop until @sevos\n"
                        ...
                        if inPuTMsG.strip() == '@sevos':
                            if evo_cycle_task and not evo_cycle_task.done():
                                evo_cycle_running = False
                                evo_cycle_task.cancel()
                                success_msg = f"[B][C][00FA9A]✅ SUCCESS! Evolution emote cycle stopped successfully!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
...
# FIXED HELP MENU SYSTEM - Now detects commands properly
                        ...
                        if inPuTMsG.strip().lower() in ("hasib", "/help", "hi", "x", "1"):
                            ...
                            # Header
                            header = f"[b][c][00BFFF] Hey User Welcome [00FFFF] To dev yt   ˣ BOT"
                            await safe_send_message(response.Data.chat_type, header, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Group Commands ─────
                            group_commands = """[B][C][FF0000]GROUP CMDS
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━
║═[B][C][00FF00]/5      
╚═[FFFFFF]5 PLAYER GROUP
║═[B][C][00FF00]/3      
╚═[B][C][FFFFFF]3 PLAYER GROUP
║═[B][C][00FF00]/6      
╚═[FFFFFF]6 PLAYER GROUP
║═[B][C][00FF00]/4 
╚═[B][C][FFFFFF]4 PLAYER GROUP
║═[B][C][00FF00]/inv (uid)      
╚═[FFFFFF]Invite Player
║═[B][C][00FF00]/exit     
╚═[B][C][FFFFFF]LEAVE GROUP
║═[B][C][00FF00]/love (name)      
╚═[FFFFFF]LOVE SPAM"""
                            ...
                            # ───── Advanced Commands ─────
                            advanced_commands = """
[B][C][FF0000] SPAM CMDS
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━
║═[B][C][FF0000]/gali (name)    
╚═[FFFFFF]GALI YOUR FRIEND
║═[B][C][FF0000]/praisa (name)    
╚═[B][C][FFFFFF]VALO GALI YOU
║═[B][C][FF0000] noob     
╚═[FFFFFF] TYPE NOOB 
║═[B][C][FF0000]/luck
╚═[B][C][FFFFFF]TRY YOUR LUCK
║═[B][C][FF0000]h (Team code)      
╚═[FFFFFF]BOT JOIN YOUR GROUP
║═[B][C][FF0000]/msg (msg) (count)  
╚═[B][C][FFFFFF]MSG SPAM"""
                            ...
                            # ───── Emote Commands ─────
                            emote_commands = """
[B][C][FF0000] EMOTE CMDS
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━
║═[B][C][FFA500]/e (name)
╚═[FFFFFF]Use Single Emote
║═[B][C][FFA500]/play (uid) 433
╚═[B][C][FFFFFF]Use Single Emote
║═[B][C][FFA500]/fast (uid) (1-21)     
╚═[FFFFFF]Fast evo (25x)
║═[B][C][FFA500]@evos (uid)
╚═[B][C][FFFFFF]EVO EMOTE CYCLE
║═[B][C][FFA500]@sevos      
╚═[FFFFFF]STOP EVO CYCLE
║═[B][C][FFA500]/sticker  
╚═[B][C][FFFFFF]BOT STICKER"""
                            await safe_send_message(response.Data.chat_type, emote_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Evolution Emote Commands ─────
                            evo_commands = """
[B][C][FF0000] EVO EMOTE CMDS
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━
║═[B][C][FFD700]/evo_c (uid) (1-21) (x)    
╚═[FFFFFF]Custom Evo (x times)
║═[B][C][FFD700]/evo (uid) (1-21)   
╚═[B][C][FFFFFF]Send Evolution Emote
║═[B][C][FFD700]/quick {code} {emote-id} {uid}   
╚═[FFFFFF] Quick Emote 
║═[B][C][FFD700]/ai {question}
╚═[B][C][FFFFFF]Ask Ai Anything 
║═[B][C][FFD700]/ms (text)     
╚═[FFFFFF]Send custom spam msg
║═[B][C][FFD700]/msg (text) (count)  
╚═[B][C][FFFFFF]MSG SPAM"""
...
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━
║═[B][C][00FA9A]/v1 (uid)
╚═[FFFFFF]Join Req Craftland
║═[B][C][00FA9A]/v2 (uid) 
╚═[B][C][FFFFFF]Join Req New VBadge
║═[B][C][00FA9A]/v3 (uid)
╚═[FFFFFF] Join Req Mdrtr Badge 
║═[B][C][00FA9A]/v4 (uid)
╚═[B][C][FFFFFF]Join Req VBadge
║═[B][C][00FA9A]/v5 (uid)   
╚═[FFFFFF]Join Req Pro Badge
║═[B][C][00FA9A]/spam (uid)  
╚═[B][C][FFFFFF]Join Req All Badge"""
                            await safe_send_message(response.Data.chat_type, ai_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Badges Commands ─────
                            badge_commands = """
[B][C][FF0000] INFO & BUNDLE
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━
║═[B][C][00FFFF]/bundle
╚═[FFFFFF]SHOW BUNDLE LIST
║═[B][C][00FFFF]/bundle (name)
╚═[B][C][FFFFFF] EQUIP RARE BUNDLE
║═[B][C][00FFFF]/lag (tc)
╚═[FFFFFF] LAG YOUR TEAM
║═[B][C][00FFFF]/like (uid)
╚═[B][C][FFFFFF] 100 LIKES 
║═[B][C][00FFFF]/status (uid) 
╚═[FFFFFF]CHECK TEAM PLAYER
║═[B][C][00FFFF]/info (uid)  
╚═[B][C][FFFFFF]CHECK PLAYER INFO"""
                            await safe_send_message(response.Data.chat_type, badge_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
                            
                                                        # ───── info Commands ─────
                            info_commands = """[B][C][FF0000]✿[00FF00]━━━━━━━━━━━━━━[FF0000]✿
[00FFFF]      WELCOME  TO  Ꮋꫝsɪʙ Cᴏᴅᴇx  BOT
[FF0000]✿[00FF00]━━━━━━━━━━━━━━[FF0000]✿
[FFFFFF]        Welcome To The Squad!
[FFFF00]        Use [00FF00]/help [FFFF00]to see menu
[00FFFF]        Developer : ㅤdev yt ㅤᏕ!Ꮢ """
